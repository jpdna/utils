...
class MetricsListener(val metrics: RecordedMetrics) extends SparkListener {
  ...
  override def onStageCompleted(stageCompleted: SparkListenerStageCompleted): Unit = {
    ...
    sparkMetrics.mapStageIdToName(stageInfo.stageId, stageInfo.name)
    getStageDuration(stageInfo).foreach(sparkMetrics.recordStageDuration(stageInfo.stageId, Option(stageInfo.name), _))
  }
  ...
  override def onTaskEnd(taskEnd: SparkListenerTaskEnd): Unit = {
    ...
    taskMetrics.foreach(e => {
      sparkMetrics.executorRunTime += e.executorRunTime
      sparkMetrics.executorDeserializeTime += e.executorDeserializeTime
      sparkMetrics.resultSerializationTime += e.resultSerializationTime
    })

    taskInfo.foreach(e => {
      sparkMetrics.duration += e.duration
    })
